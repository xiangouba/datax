...
public class OssWriter extends Writer {
    ...
    public static class Job extends Writer.Job {
        ...
        private void validateParameter() {
            ...
            logger.info("writeSingleObject is: {}", this.writeSingleObject);
        }

        @Override
        public void prepare() {
            logger.info("begin do prepare...");
            if(useHdfsWriterProxy){
            ...
            if (this.isPeer2PeerCopyMode()) {
                ...
                if ("prefix".equalsIgnoreCase(truncateMode)) {
                    ...
                    if (TRUNCATE.equals(writeMode)) {
                        logger.info("You have configured [writeMode] [truncate], so the system will start to clear the objects starting with [{}] under [{}]. ", bucket, object);
                        // warn: 默认情况下，如果Bucket中的Object数量大于100，则只会返回100个Object
                        while (true) {
                            ObjectListing listing = null;
                            logger.info("list objects with listObject(bucket, object)");
                            ...
                            logger.info(String.format("[prefix truncate mode]delete oss object [%s].", JSON.toJSONString(objects2Delete)));
                            ...
                            logger.warn("OSS request id:{}, objects delete failed:{}", deleteResult.getRequestId(),
                    ...
                    if (TRUNCATE.equals(writeMode)) {
                        ...
                        for (List<String> eachSlice : splitResult) {
                            ...
                            logger.info(String.format("[origin object name truncate mode]delete oss object [%s].", JSON.toJSONString(ossObjFullPath)));
                            DeleteObjectsRequest deleteRequest = new DeleteObjectsRequest(bucket);
                ...
                try {
                    ...
                    logger.info(String.format("access control details [%s].",
        ...
        private void doPrepareForSingleObject(String bucket, String object, String writeMode) {
            ...
            logger.info("does object [{}] exist in bucket {} : {}", object, bucket, doesObjectExist);
            if (TRUNCATE.equals(writeMode)) {
                logger.info("Because you have configured writeMode truncate, and writeSingleObject is true, start cleaning up the duplicate object [{}] under [{}]", bucket, object);
                if (doesObjectExist) {
                    logger.info("object [{}] has exist in bucket, delete it!", object, bucket);
                ...
                logger.info("Because you have configured writeMode nonConflict, and writeSingleObject is true, start checking bucket [{}] under the same name object [{}]", bucket, object);
                if (doesObjectExist) {
                    throw DataXException
                            .asDataXException(
                                    OssWriterErrorCode.ILLEGAL_VALUE,
            ...
            if (TRUNCATE.equals(writeMode)) {
                ...
                while (true) {
                    ...
                    List<OSSObjectSummary> objectSummarys = listing
                            .getObjectSummaries();
                    for (OSSObjectSummary objectSummary : objectSummarys) {
                        logger.info(String.format("delete oss object [%s].",
                ...
                logger.info("You have configured [writeMode] [append], so the system won\\u2019t perform the clearing before writing. Data is written to objects with the name prefix of [{}] under the bucket: [{}]. ", bucket, object);
            } else if (NOCONFLICT.equals(writeMode)) {
                logger.info("You have configured [writeMode] [nonConflict], so the system will start to check objects whose names start with [{}] under the bucket: [{}]. ", bucket, object);
        ...
        public void post() {
            ...
            if (this.writeSingleObject) {
                try {
                    /**1. 合并上传最后一个block*/
                    logger.info("Has upload part size: {}", OssSingleObject.allPartETags.size());
                    if (OssSingleObject.getLastBlockBuffer() != null && OssSingleObject.getLastBlockBuffer().length != 0) {
                        ...
                        logger.info("post writer single object last merge block size is : {}", byteBuffer.length);
                    ...
                    if (OssSingleObject.allPartETags.size() == 0) {
                        logger.warn("allPartETags size is 0, there is no part of data need to be complete uploaded, "
                    ...
                    /**2. 完成complete upload */
                    logger.info("begin complete multi part upload, bucket:{}, object:{}, uploadId:{}, all has upload part size:{}",
                    ...
                    logger.info(String.format("post final object etag is:[%s]", completeMultipartUploadResult.getETag()));
                } catch (Exception e) {
                    logger.error("osswriter post error: {}", e.getMessage(), e);
        ...
        public void destroy() {
            ...
            try {
                ...
                logger.warn("shutdown ossclient meet a exception:" + e.getMessage(), e);
            }
        }
        ...
        public List<Configuration> split(int mandatoryNumber) {
            ...
            logger.info("end do split. split size: {}", writerSplitConfigs.size());
        ...
        private List<Configuration> doSplitForWriteSingleObject(int mandatoryNumber) {
            logger.info("writeSingleObject is true, begin do split for write single object.");
            List<Configuration> writerSplitConfigs = new ArrayList<Configuration>();
            ...
            try {
                ...
                logger.error("initiateMultipartUpload error: {}", e.getMessage(), e);
                throw DataXException.asDataXException(e.getMessage());
            }
            /**
            ...
            logger.info("writeSingleObject use uploadId: {}", uploadId);
        ...
        private List<Configuration> doSplitForWriteMultiObject(int mandatoryNumber) {
            ...
            for (int i = 0; i < mandatoryNumber; i++) {
                ...
                logger.info(String.format("splited write object name:[%s]",
                        fullObjectName));
    ...
    public static class Task extends Writer.Task {
        ...
        public void startWriteSingleObjectUnstructedStorageFile(RecordReceiver lineReceiver) {
            ...
            try {
                ...
                if (sb.length() > 0) {
                    ...
                    logger.info("begin add last block to buffer, last block size: {}", lastBlock.length);
            ...
            logger.info("single oss object end do write");
        }
        ...
        private void startWriteBinaryFile(RecordReceiver lineReceiver) {
            ...
            try {
                ...
                while ((record = lineReceiver.getFromReader()) != null) {
                    ...
                    if (needInitMultipartTransform || !StringUtils.equals(currentObject, fullObjectNameTmp)) {
                        ...
                        if (null != currentInitiateMultipartUploadResult) {
                            ...
                            logger.info(String.format("final object [%s] etag is:[%s]", commitKey,
                        ...
                        logger.info(String.format("write to bucket: [%s] object: [%s] with oss uploadId: [%s]",
                ...
                if (!gotData) {
                    logger.info("Receive no data from the source.");
                ...
                if (!StringUtils.equals(lastUploadId, currentInitiateMultipartUploadResult.getUploadId())) {
                    ...
                    logger.info(String.format("final object etag is:[%s]", completeMultipartUploadResult.getETag()));
                }
            } catch (IOException e) {
                // 脏数据UnstructuredStorageWriterUtil.transportOneRecord已经记录,header
                // 都是字符串不认为有脏数据
                ...
                if (!StringUtils.equals(lastUploadId, currentInitiateMultipartUploadResult.getUploadId())) {
                    ...
                    if (gotData) {
                        ...
                        if (generateEmptyFile) {
                            logger.info("Due to without data, oss will generate empty file, " +
                                    "the generateEmptyFile is {}, you can set it false to avoid this",generateEmptyFile);
                            completeUpload(completeMultipartUploadRequest);
                        } else {
                            logger.info("The generateEmptyFile is false, datax will not generate empty file");
                        }
                    }
                }
            } catch (IOException e) {
        ...
        private void completeUpload(CompleteMultipartUploadRequest completeMultipartUploadRequest) throws Exception {
            CompleteMultipartUploadResult completeMultipartUploadResult = this.ossWriterProxy.completeMultipartUpload(completeMultipartUploadRequest);
            logger.info(String.format("final object etag is:[%s]",
