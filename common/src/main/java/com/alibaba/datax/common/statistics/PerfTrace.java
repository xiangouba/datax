...
public class PerfTrace {
    ...
    private static Logger logger = LoggerFactory.getLogger(PerfTrace.class);
    ...
    public static PerfTrace getInstance() {
        if (instance == null) {
            logger.error("PerfTrace instance not be init! must have some error! ");
            synchronized (lock) {
                if (instance == null) {
    ...
    private PerfTrace(boolean isJob, long jobId, int taskGroupId, int priority, boolean enable) {
        try {
            ...
            logger.info(String.format("PerfTrace traceId=%s, isEnable=%s, priority=%s", this.perfTraceId, this.enable, this.priority));
