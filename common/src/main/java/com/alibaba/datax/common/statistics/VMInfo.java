 ...
 */
public class VMInfo {
    private static final Logger logger = LoggerFactory.getLogger(VMInfo.class);
            ...
            synchronized (lock) {
                if (vmInfo == null) {
                    try {
                        ...
                        logger.warn("no need care, the fail is ignored : vmInfo init failed " + e.getMessage(), e);
                    }
                }
            }
    ...
    private VMInfo() {
        ...
        logger.info("VMInfo# operatingSystem class => " + osMXBean.getClass().getName());
    ...
    public synchronized void getDelta(boolean print) {
        ...
        try {
            ...
            if (print) {
                logger.info(processCpuStatus.getDeltaString() + processMomoryStatus.getDeltaString() + processGCStatus.getDeltaString());
            }

        } catch (Exception e) {
            logger.warn("no need care, the fail is ignored : vmInfo getDelta failed " + e.getMessage(), e);
        }
    }
    ...
    public static long getLongFromOperatingSystem(OperatingSystemMXBean operatingSystem, String methodName) {
        try {
            ...
            logger.info(String.format("OperatingSystemMXBean %s failed, Exception = %s ", methodName, e.getMessage()));
        }
