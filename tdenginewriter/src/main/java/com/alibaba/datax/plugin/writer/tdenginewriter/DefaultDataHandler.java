...
public class DefaultDataHandler implements DataHandler {
    ...
    public int handle(RecordReceiver lineReceiver, TaskPluginCollector collector) {
        ...
        try (Connection conn = DriverManager.getConnection(jdbcUrl, username, password)) {
            logger.info("connection[ jdbcUrl: " + jdbcUrl + ", username: " + username + "] established.");
            ...
            for (int i = 1; (record = lineReceiver.getFromReader()) != null; i++) {
                if (i % batchSize != 0) {
                    ...
                    try {
                        ...
                        logger.warn("use one row insert. because:" + e.getMessage());
        ...
        if (affectedRows != count) {
            logger.error("write record missing or incorrect happened, affectedRows: " + affectedRows + ", total: " + count);
        }
...
//                        return colMeta.field + "=" + record.getColumn(indexOf(colMeta.field)).asString();
        ...
        logger.warn("schemalessWriter does not return affected rows!");
        return count;
    }
...
//            }).collect(Collectors.joining(", ", "(", ") ")));
        ...
        if (validRecords == 0) {
            logger.warn("no valid records in this batch");
            return 0;
        }
