...
public class Kudu11xHelper {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Kudu11xHelper.class);
        ...
        try {
            ...
            while (i.get() > 0) {
                try {
                    ...
                    logger.info("Wait for the table to be created..... " + i);
                    try {
                        Thread.sleep(100L);
            ...
            try {
                ...
                logger.info("Kudu client has been shut down!");
            }
        }
    }
    ...
    public static void closeClient(KuduClient kuduClient) {
        try {
            ...
            logger.warn("The \"kudu client\" was not stopped gracefully. !");
    ...
    public static void setTablePartition(Configuration configuration,
        ...
        if (range != null) {
            ...
            logger.info("Set range partition complete!");
        }
        ...
        if (hash != null) {
            ...
            logger.info("Set hash partition complete!");
        }
    }

    public static void validateParameter(Configuration configuration) {
        logger.info("Start validating parameters！");
...
//        logger.info("------------------------------------");
        logger.info("validate parameter complete！");
    }

    public static void truncateTable(Configuration configuration) {
        String kuduConfig = configuration.getString(Key.KUDU_CONFIG);
        String userTable = configuration.getString(Key.TABLE);
        logger.info(String.format("Because you have configured truncate is true,KuduWriter begins to truncate table %s .", userTable));
        KuduClient kuduClient = Kudu11xHelper.getKuduClient(kuduConfig);

        try {
            if (kuduClient.tableExists(userTable)) {
                kuduClient.deleteTable(userTable);
                logger.info(String.format("table  %s has been deleted.", userTable));
            }
        } catch (KuduException e) {
