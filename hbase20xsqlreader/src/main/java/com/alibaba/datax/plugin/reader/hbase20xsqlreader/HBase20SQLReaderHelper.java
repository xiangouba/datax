...
public class HBase20SQLReaderHelper {
    private static final Logger logger = LoggerFactory.getLogger(HBase20SQLReaderHelper.class);
    ...
    public void validateParameter() {
        ...
        if (querySql == null || querySql.isEmpty()) {
            logger.info("Split according to splitKey or split points.");
            ...
            logger.info("Split according to query sql.");
        }
    }
    ...
    public Connection getConnection(String queryServerAddress, String serialization) {
        ...
        logger.debug("Connecting to QueryServer [" + url + "] ...");
        Connection conn;
        try {
        ...
        logger.debug("Connected to QueryServer successfully.");
        return conn;
    }
    ...
    public void closeJdbc(Connection connection, Statement statement, ResultSet resultSet) {
        try {
            ...
            logger.warn("数据库连接关闭异常.", HBase20xSQLReaderErrorCode.CLOSE_PHOENIX_CONNECTION_ERROR, e);
        }
    }
    ...
    public List<Configuration> doSplit(int adviceNumber) {
        ...
        if (querySql == null || querySql.isEmpty()) {
            ...
            if (splitPoints == null || splitPoints.isEmpty()) {
                logger.info("Split accoring min and max value of splitColumn...");
                ...
                logger.info("Split accoring splitPoints...");
                // 根据指定splitPoints进行切分
            ...
            if (null != rangeList && !rangeList.isEmpty()) {
                for (String range : rangeList) {
                    ...
                    logger.info("Query SQL: " + tempQuerySql);
