    ...
    public static <V> V executeWithRetry(Callable<V> callable, int maxRetryTimes, int sleepInMilliSecond) throws Exception {
        ...
        while (true){
            ...
            try {
                ...
                logger.warn("Call callable fail, {}", e.getMessage());
                if (!canRetry(e)){
                    logger.error("Can not retry for Exception.", e); 
                    throw e;
                } else if (retryTimes >= maxRetryTimes) {
                    logger.error("Retry times more than limition. maxRetryTimes : {}", maxRetryTimes); 
                    throw e;
                }
                retryTimes++;
                logger.warn("Retry time : {}", retryTimes);
            }
        }
    }
