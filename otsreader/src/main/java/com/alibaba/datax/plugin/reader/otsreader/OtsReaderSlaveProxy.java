...
public class OtsReaderSlaveProxy {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OtsReaderSlaveProxy.class);
    ...
    private void rowsToSender(List<Row> rows, RecordSender sender, List<OTSColumn> columns) {
        for (Row row : rows) {
            ...
            logger.debug("Reader send record : {}", line.toString());
    ...
    public void read(RecordSender sender, Configuration configuration) throws Exception {
        ...
        do {
            logger.debug("Next token : {}", GsonParser.rowPrimaryKeyToJson(token));
            if (request == null) {
