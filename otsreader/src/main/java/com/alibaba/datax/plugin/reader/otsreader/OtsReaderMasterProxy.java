...
public class OtsReaderMasterProxy {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OtsReaderMasterProxy.class);
    ...
    public void init(Configuration param) throws Exception {        
        ...
        logger.info("Table Meta : {}", GsonParser.metaToJson(meta));
    ...
    public List<Configuration> split(int num) throws Exception {
        logger.info("Expect split num : " + num);
        ...
        if (this.conf.getRangeSplit() != null) { // 用户显示指定了拆分范围
            logger.info("Begin userDefinedRangeSplit");
            ranges = userDefinedRangeSplit(meta, range, this.conf.getRangeSplit());
            logger.info("End userDefinedRangeSplit");
        } else { // 采用默认的切分算法 
            logger.info("Begin defaultRangeSplit");
            ranges = defaultRangeSplit(ots, meta, range, num);
            logger.info("End defaultRangeSplit");
        }
        ...
        logger.info("Configuration list count : " + configurations.size());
    ...
    private List<OTSRange> defaultRangeSplit(OTSClient ots, TableMeta meta, OTSRange range, int num) throws Exception {
        ...
        if (realDirection != direction) {
            logger.warn("Expect '" + direction + "', but direction of realBegin and readlEnd is '" + realDirection + "'");
            List<OTSRange> ranges = new ArrayList<OTSRange>();
            ranges.add(range);
            return ranges;
        }
