...
public class HbaseSQLHelper {
    private static final Logger logger = LoggerFactory.getLogger(HbaseSQLHelper.class);
    ...
    public static Connection getJdbcConnection(HbaseSQLWriterConfig cfg) {
        ...
        logger.debug("Connecting to HBase cluster [" + connStr + "] ...");
        Connection conn;
        try {
        ...
        logger.debug("Connected to HBase cluster successfully.");
        return conn;
    }
    ...
    public static Connection getThinClientJdbcConnection(HbaseSQLWriterConfig cfg) throws SQLException {
        String connStr = cfg.getConnectionString();
        logger.info("Connecting to HBase cluster [" + connStr + "] use thin client ...");
    ...
    public static PTable getTableSchema(Connection conn, String namespace, String fullTableName, boolean isThinClient)
        throws
        SQLException {
        logger.info("Start to get table schema of namespace=" + namespace + " , fullTableName=" + fullTableName);
        if (!isThinClient) {
