...
public class HbaseSQLWriterTask {
    private final static Logger logger = LoggerFactory.getLogger(HbaseSQLWriterTask.class);
    ...
    private void close()  {
        if (ps != null) {
            try {
                ...
                // 不会出错
                logger.error("Failed closing PreparedStatement", e);
            }
        }
        if (connection != null) {
            try {
    ...
    private void doBatchUpsert(List<Record> records) throws SQLException {
        try {
            ...
            logger.error("Failed batch committing " + records.size() + " records", e);
    ...
    private void doSingleUpsert(List<Record> records) throws SQLException {
        for (Record r : records) {
            try {
                ...
                //出错了，记录脏数据
                logger.error("Failed writing hbase", e);
    ...
    private PreparedStatement createPreparedStatement() throws SQLException {
        ...
        logger.debug("SQL template generated: " + sql);
        return ps;
    }
    ...
    private int[] getColumnSqlType(List<String> columnNames) throws SQLException {
        ...
        for (int i = 0; i < columnNames.size(); i++) {
            ...
            logger.debug("Column name : " + name + ", sql type = " + type.getSqlType() + " " + type.getSqlTypeName());
        }
        return types;
    }
