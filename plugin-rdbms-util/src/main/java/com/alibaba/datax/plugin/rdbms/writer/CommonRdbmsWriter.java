...
public class CommonRdbmsWriter {
    ...
    public static class Task {
            ...
            try {
                ...
                while ((record = recordReceiver.getFromReader()) != null) {
                    ...
                    if (writeBuffer.size() >= batchSize || bufferBytes >= batchByteSize) {
                        if (deleteFlag){
                            doBatchDelete(connection,writeBuffer);
                        }
        ...
        public void doBatchDelete(Connection connection,List<Record> buffer) throws SQLException {
            //删除sql
            calcDeleteRecordSql();
            ...
            for (Record record : buffer) {
                //删除的数据
                deleteList.add(JSONObject.parseObject(record.getColumn(0).getRawData().toString().toLowerCase(Locale.ROOT)));
            }
            //删除的参数
            StringBuilder paramSql = new StringBuilder();
            for (Map<String, Object> objectMap : deleteList) {
                paramSql.append(objectMap.get("id".toLowerCase(Locale.ROOT))).append("','");
            }
            //删除拼接后多余的最后的两个字符
            ...
            try {
                ...
                this.deleteRecordSql = this.deleteRecordSql + paramSql;
                //DELETE FROM %s WHERE %s in ('
                preparedStatement = connection
                        .prepareStatement(this.deleteRecordSql);
                preparedStatement.executeUpdate();
                connection.commit();
            } catch (SQLException e) {
                logger.warn("回滚此次写入,删除报错. 因为:" + e.getMessage());
                connection.rollback();
            } catch (Exception e) {
                throw DataXException.asDataXException(
                        DBUtilErrorCode.WRITE_DATA_ERROR, e);
