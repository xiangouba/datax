...
public class HintUtil {
    private static final Logger logger = LoggerFactory.getLogger(ReaderSplitUtil.class);
            ...
            if(tablePattern != null && DataBaseType.Oracle.equals(dataBaseType)) {
                ...
                if(m.find()){
                    ...
                    if(finalHint.indexOf("parallel") > 0 && DBUtil.isOracleMaster(jdbcUrl, username, password)){
                        logger.info("master:{} will not use hint:{}", jdbcUrl, finalHint);
                    }else{
                        logger.info("table:{} use hint:{}.", table, finalHint);
                        return finalHint + column;
                    }
                }
            }
        } catch (Exception e){
            logger.warn("match hint exception, will not use hint", e);
        }
        return column;
    }
