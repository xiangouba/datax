...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    public static int delete(Connection conn, String sql, int fetchSize)
            throws SQLException {
        // 默认3600 s 的query Timeout
        return delete(conn, sql, fetchSize, Constant.SOCKET_TIMEOUT_INSECOND);
    }
    ...
    public static int delete(Connection conn, String sql, int fetchSize, int queryTimeout)
            throws SQLException {
        // make sure autocommit is off
        conn.setAutoCommit(false);
        Statement stmt = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY,
