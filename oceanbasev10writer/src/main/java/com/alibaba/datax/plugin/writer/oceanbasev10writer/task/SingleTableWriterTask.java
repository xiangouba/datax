...
public class SingleTableWriterTask extends CommonRdbmsWriter.Task {
	...
	public void init(Configuration config) {
		...
		logger.info("In Write OceanBase 1.0, Real Batch Size : " + this.batchSize);

		isOracleCompatibleMode = ObWriterUtils.isOracleMode();
		logger.info("isOracleCompatibleMode=" + isOracleCompatibleMode);
		...
		try {
			for (int i = 0; i < failTryCount; i++) {
				...
				try {
					...
					if (fatalFail) {
						logger.warn("Fatal exception in OB. Roll back this write and hibernate for five minutes. SQLState: {}. ErrorCode: {}",
						...
						logger.warn("Recoverable exception in OB. Roll back this write and hibernate for one minute. SQLState: {}. ErrorCode: {}",
								e.getSQLState(), e.getErrorCode(), e);
						conn.rollback();
						ObWriterUtils.sleep(60000);
						// 其它异常直接退出,采用逐条写入方式
					} else {
						logger.warn("Exception in OB. Roll back this write and hibernate for one second. Write and submit the records one by one. SQLState: {}. ErrorCode: {}",
								e.getSQLState(), e.getErrorCode(), e);
						conn.rollback();
			...
			logger.warn("Exception in OB. Roll back this write. Write and submit the records one by one. SQLState: {}. ErrorCode: {}",
					e.getSQLState(), e.getErrorCode(), e);
		}
		if (!success) {
	...
	private void checkMemstore(Connection conn) {
		...
		while (ObWriterUtils.isMemstoreFull(conn, memstoreThreshold)) {
			logger.warn("OB memstore is full,sleep 60 seconds, threshold=" + memstoreThreshold);
			ObWriterUtils.sleep(60000);
		}
