...
public class OBDataSourceV10 {
	private static final Logger logger = LoggerFactory.getLogger(OBDataSourceV10.class);
	...
	public static synchronized void init(Configuration configuration,
		...
		if (dataSources.containsKey(dataSourceKey)) {
			...
			if (ocjGetConnectionTimeout == 0) {
				...
				logger.info(String.format("initializing OCJ with ocjGetConnectionTimeout=%d, " +
	...
	public static synchronized void destory(final String dataSourceKey){
		...
		if (holder.canClose()) {
			...
			holder.close();
			logger.info(String.format("close datasource success [%s]", dataSourceKey));
		}
	}
	...
	private static class DataSourceHolder {
		...
		public DataSourceHolder(final String rsUrl,
		    ...
		    logger.info("Create GroupDataSource rsUrl=[{}], fullUserName=[{}], dbName=[{}], getConnectionTimeout= {}ms, maxActive={}",
