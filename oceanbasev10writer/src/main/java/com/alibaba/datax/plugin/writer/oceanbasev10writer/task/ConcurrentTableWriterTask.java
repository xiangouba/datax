...
public class ConcurrentTableWriterTask extends CommonRdbmsWriter.Task {
    private static final Logger logger = LoggerFactory.getLogger(ConcurrentTableWriterTask.class);
...
//	private List<Record> unknownPartRecords;
	...
	public void init(Configuration config) {
        ...
        logger.info("configure url is unavailable, use obclient for connections.");
        ...
        if (isOracleCompatibleMode) {
			...
			logger.info(String.format("this is oracle compatible mode, change database to %s, table to %s",
        ...
        if (config.getBool(Config.USE_PART_CALCULATOR, Config.DEFAULT_USE_PART_CALCULATOR)) {
            ...
            logger.info("Disable partition calculation feature.");
        }
	...
	private void initPartCalculator(ServerConnectInfo connectInfo) {
		int retry = 0;
		logger.info(String.format("create tableEntryKey with clusterName %s, tenantName %s, databaseName %s, tableName %s",
		...
		do {
			try {
				if (retry > 0) {
					...
					logger.info("retry create new part calculator, the {} times", retry);
				}
				logger.info("create partCalculator with address: " + connectInfo.ipPort);
				partCalculator = new ObPartitionIdCalculator(connectInfo.ipPort, tableEntryKey);
			} catch (Exception ex) {
				++retry;
				logger.warn("create new part calculator failed, retry {}: {}", retry, ex.getMessage());
			}
	...
	public void setPutAllTaskInQueue() {
		...
		logger.info("ConcurrentTableWriter has put all task in queue, queueSize = {},  total = {}, finished = {}",
	...
	private void rewriteSql() {
		...
		logger.info("writeRecordSql :{}", this.writeRecordSql);
	}
	...
	private void calPartitionKeyIndex(List<Integer> partKeyIndexes) {
		...
		if (null == partCalculator) {
			logger.error("partCalculator is null");
			return;
		}
		for (int i = 0; i < columns.size(); ++i) {
			if (partCalculator.isPartitionKeyColumn(columns.get(i))) {
			    logger.info(columns.get(i) + " is partition key.");
        ...
        do {
        	try {
        		if (retryTimes > 0) {
        			...
        			logger.warn("getColumnMetaData of table {} failed, retry the {} times ...", this.table, retryTimes);
        		}
        		...
        		++retryTimes;
        		e.printStackTrace();
        		logger.warn("fetch column meta of [{}] failed..., retry {} times", this.table, retryTimes);
        	} catch (InterruptedException e) {
				logger.warn("startWriteWithConnection interrupt, ignored");
			} finally {
        	}
        ...
        try {
            ...
            while ((record = recordReceiver.getFromReader()) != null) {
                if (record.getColumnNumber() != this.columnNumber) {
                	...
                	logger.error("column not equal {} != {}, record = {}",
	...
	private void addRecordToCache(final Record record) {
		...
		try {
		    ...
		    logger.warn("fail to get partition id: " + e1.getMessage() + ", record: " + record);
		}

        if (partId == null && isOb2) {
            logger.debug("fail to calculate parition id, just put into the default buffer.");
		...
		if (partId != null) {
			...
			logger.debug("add unknown part record {}", record);
	...
	private List<Record> addRecordsToWriteQueue(List<Record> records) {
		...
		while (true) {
			if (i > 0) {
				logger.info("retry add batch record the {} times", i);
			}
			try {
				...
				i++;
				logger.info("Concurrent table writer is interrupted");
			}
		}
	...
	private void checkMemStore() {
		...
		if (isFull) {
			logger.warn("OB memstore is full,sleep 30 seconds, threshold=" + memstoreThreshold);
		}
	...
	public void print() {
		logger.debug("Statistic total task {}, finished {}, queue Size {}",
				concurrentWriter.getTotalTaskCount(),
				concurrentWriter.getFinishTaskCount(),
				concurrentWriter.getTaskQueueSize());
	...
	public void waitTaskFinish() {
		...
		try {
			...
			logger.warn("Concurrent table writer wait task finish interrupt");
		} finally {
			lock.unlock();
		}
		logger.debug("wait all InsertTask finished ...");
	}
	...
	public class ConcurrentTableWriter {
		...
		public synchronized void start() {
			for (int i = 0; i < threadCount; ++i) {
			    logger.info("start {} insert task.", (i+1));
		...
		public void printStatistics() {
			...
			ConcurrentTableWriterTask.logger.debug("Insert {} times, totalCost {} ms, average {} ms",
