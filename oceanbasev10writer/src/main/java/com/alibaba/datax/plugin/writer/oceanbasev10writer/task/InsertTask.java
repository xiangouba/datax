...
public class InsertTask implements Runnable {
	...
	public void run() {
		...
		logger.debug("Task {} start to execute...", taskId);
		while (!isStop()) {
			try {
				...
				if (null != records) {
					...
					logger.debug("not more task, thread exist ...");
					break;
				} else {
					TimeUnit.MILLISECONDS.sleep(5);
				}
			} catch (InterruptedException e) {
				logger.debug("TableWriter is interrupt");
			} catch (Exception e) {
				logger.warn("ERROR UNEXPECTED {}", e);
			}
		}
		logger.debug("Thread exist...");
	}
	...
	public void calStatistic(final long cost) {
		...
		if (this.printCost && cost > this.costBound) {
			logger.info("slow multi insert cost {}ms", cost);
		}
	}
	...
	private void doDelete(Connection conn, final List<Record> buffer) throws SQLException {
		...
		for (int i = 0; i < deleteMeta.size(); i++) {
			...
			try {
				...
				logger.debug("delete values: " + builder.toString());
				ps.executeBatch();
			} catch (SQLException ex) {
				logger.error("SQL Exception when delete records with {}", deleteSql, ex);
				throw ex;
			} finally {
			...
			for (int i = 0; i < failTryCount; ++i) {
				if (i > 0) {
					try {
						...
						logger.info("thread interrupted ..., ignore");
					}
					conn = connHolder.getConn();
					logger.info("retry {}, start do batch insert, size={}", i, buffer.size());
					checkMemstore();
				}
				...
				try {
					...
					logger.warn("Insert fatal error SqlState ={}, errorCode = {}, {}", e.getSQLState(), e.getErrorCode(), e);
					if (i == 0 || i > 10 ) {
						for (Record record : buffer) {
					...
					e.printStackTrace();
					logger.warn("Insert error unexpected {}", e);
				} finally {
					DBUtil.closeDBResources(ps, null);
				}
			}
		} catch (SQLException e) {
			logger.warn("ERROR:retry failSql State ={}, errorCode = {}, {}", e.getSQLState(), e.getErrorCode(), e);
		}

		if (!success) {
			try {
				logger.info("do one insert");
