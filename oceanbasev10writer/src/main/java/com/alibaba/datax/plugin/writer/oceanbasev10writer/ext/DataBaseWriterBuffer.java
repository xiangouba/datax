...
public class DataBaseWriterBuffer {
	private static final Logger logger = LoggerFactory.getLogger(DataBaseWriterBuffer.class);
	...
	public synchronized void checkMemstore(long memstoreCheckIntervalSecond, double memstoreThreshold) {
		...
		logger.debug(String.format("checking memstore usage: lastCheckTime=%d, now=%d, check interval=%d, threshold=%f",
		...
		while (ObWriterUtils.isMemstoreFull(conn, memstoreThreshold)) {
			logger.warn("OB memstore is full,sleep 60 seconds, jdbc=" + getConnHolder().getJdbcUrl()
