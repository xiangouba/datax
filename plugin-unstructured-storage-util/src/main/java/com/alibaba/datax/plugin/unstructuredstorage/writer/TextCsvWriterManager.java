        ...
        if (StringUtils.isNotBlank(csvWriterConfig)) {
            try {
                ...
                if (!csvWriterConfigMap.isEmpty()) {
                    ...
                    logger.info(String.format("csvwriterConfig is set successfully. After setting, csvwriter:%s", JSON.toJSONString(this.csvWriter)));
                }
            } catch (Exception e) {
                logger.warn(String.format("invalid csvWriterConfig config: %s, DataX will ignore it.", csvWriterConfig),
                        e);
            }
        }
    }
