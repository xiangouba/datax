...
public class UnstructuredStorageReaderUtil {
    private static final Logger logger = LoggerFactory.getLogger(UnstructuredStorageReaderUtil.class);
    ...
    public static void readFromStream(InputStream inputStream, String context,
        ...
        if (StringUtils.isBlank(encoding)) {
            ...
            logger.warn(String.format("您配置的encoding为[%s], 使用默认值[%s]", encoding,
                    Constant.DEFAULT_ENCODING));
        }
    ...
    public static void doReadFromStream(BufferedReader reader, String context,
        ...
        if (null == delimiterInStr) {
            logger.warn(String.format("您没有配置列分隔符, 使用默认值[%s]",
        ...
        try {
            ...
            if (skipHeader) {
                ...
                logger.info(String.format("Header line %s has been skiped.",
                        fetchLine));
            }
    ...
    public static void validateCsvReaderConfig(Configuration readerConfiguration) {
        ...
        if (StringUtils.isNotBlank(csvReaderConfig)) {
            try {
                ...
                logger.info(String.format("WARN!!!!忽略csvReaderConfig配置! 配置错误,值只能为空或者为Map结构,您配置的值为: %s", csvReaderConfig));
            }
        }
    }
    ...
    public static void setCsvReaderConfig(CsvReader csvReader) {
        if (null != UnstructuredStorageReaderUtil.csvReaderConfigMap && !UnstructuredStorageReaderUtil.csvReaderConfigMap.isEmpty()) {
            try {
                ...
                logger.info(String.format("WARN!!!!忽略csvReaderConfig配置!通过BeanUtils.populate配置您的csvReaderConfig发生异常,您配置的值为: %s;请检查您的配置!CsvReader使用默认值[%s]",
                        JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap), JSON.toJSONString(csvReader)));
            }
        } else {
            //默认关闭安全模式, 放开10W字节的限制
            csvReader.setSafetySwitch(false);
            logger.info(String.format("CsvReader使用默认值[%s],csvReaderConfig值为[%s]", JSON.toJSONString(csvReader), JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap)));
        }
    }
