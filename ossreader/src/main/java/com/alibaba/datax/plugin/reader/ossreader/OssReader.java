...
public class OssReader extends Reader {
    public static class Job extends Reader.Job {
        ...
        public List<Configuration> split(int adviceNumber) {
            ...
            if (0 == objects.size() && this.successOnNoObject) {
                ...
                logger.info(String.format("no OSS object to be read"));
                logger.debug("split() ok and end...");
            ...
            logger.info("all split done, cost {}ms", t2 - t1);
            /**
            ...
            if(readerSplitConfigs.size() < adviceNumber){
                logger.info("[Note]: During OSSReader data synchronization, one file can only be synchronized in one task. You want to synchronize {} files " +
                ...
                if(errorMessage.contains("UnknownHost")){
                    ...
                    if (e instanceof OSSException) {
                        ...
                        if ("NoSuchKey".equalsIgnoreCase(ossException
                            ...
                            logger.warn(String.format("oss file %s is not exits to read:", object), e);
                            return;
                        }
                    }
                    // object配置错误
        ...
        private List<ObjectListing> getRemoteObjectListings(String parentDir) throws OSSException, ClientException {
            ...
            logger.debug("Parent folder: {}", parentDir);
            ...
            try {
                ...
                do {
                    ...
                    if (null != remoteObjectList) {
                        logger.info("ListObjects prefix: {} requestId: {}", remoteObjectList.getPrefix(), remoteObjectList.getRequestId());
                    } else {
                        logger.info("ListObjectsRequest get null");
                    }
                    remoteObjectListings.add(remoteObjectList);
                    listObjectsRequest.setMarker(remoteObjectList.getNextMarker());
                    logger.debug(listObjectsRequest.getMarker());
                    logger.debug(String.valueOf(remoteObjectList.isTruncated()));
    ...
    public static class Task extends Reader.Task {
        private static Logger logger = LoggerFactory.getLogger(Reader.Task.class);
        ...
        @Override
        public void prepare() {
            logger.info("task prepare() begin...");
            if(useHdfsReaderProxy){
        ...
        public void startRead(RecordSender recordSender) {
            ...
            for (StartEndPair eachSlice : this.allWorksForTask) {
                ...
                logger.info(String.format("read bucket=[%s] object=[%s], range: [start=%s, end=%s] start...", bucket,
                        object, start, end));
