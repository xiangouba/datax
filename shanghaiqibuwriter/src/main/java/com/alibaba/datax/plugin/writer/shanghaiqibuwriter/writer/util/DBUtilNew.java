package com.alibaba.datax.plugin.writer.shanghaiqibuwriter.writer.util;
...
public final class DBUtilNew {
    private static final Logger logger = LoggerFactory.getLogger(DBUtilNew.class);
...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
                ...
                if ("ON".equalsIgnoreCase(readOnly)) { //备库
                    ...
                    if (DBUtilNew.asyncResultSetNext(rs1)) {
                        ...
                        if ("Yes".equalsIgnoreCase(ioRunning) && "Yes".equalsIgnoreCase(sqlRunning)) {
                            ...
                            DBUtilNew.asyncResultSetNext(rs2);
                        ...
                        logger.warn("SHOW SLAVE STATUS has no result");
                    }
                }
            } else {
                logger.warn("SHOW VARIABLES like 'read_only' has no result");
            }
        } catch (Exception e) {
            logger.warn("checkSlave failed, errorMessage:[{}].", e.getMessage());
        }
        return false;
    }
    ...
    public static boolean hasInsertPrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password, List<String> tableList) {
        ...
        try {
            ...
            while (DBUtilNew.asyncResultSetNext(rs)) {
            ...
            logger.warn("Check the database has the Insert Privilege failed, errorMessage:[{}]", e.getMessage());
        }
            ...
            try {
                ...
                if (DataBaseType.Oracle.equals(dataBaseType)) {
                    if (e.getMessage() != null && e.getMessage().contains("insufficient privileges")) {
                        ...
                        logger.warn("User [" + userName + "] has no 'insert' privilege on table[" + tableName + "], errorMessage:[{}]", e.getMessage());
                    }
                } else {
                    hasInsertPrivilege = false;
                    logger.warn("User [" + userName + "] has no 'insert' privilege on table[" + tableName + "], errorMessage:[{}]", e.getMessage());
    ...
    public static boolean checkDeletePrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password, List<String> tableList) {
        ...
        for (String tableName : tableList) {
            ...
            try {
                ...
                hasInsertPrivilege = false;
                logger.warn("User [" + userName + "] has no 'delete' privilege on table[" + tableName + "], errorMessage:[{}]", e.getMessage());
            }
        }
        try {
            connection.close();
        ...
        try {
            return RetryUtil.executeWithRetry(new Callable<Connection>() {
                ...
                public Connection call() throws Exception {
                    return DBUtilNew.connect(dataBaseType, jdbcUrl, username,
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        if (url.startsWith(com.alibaba.datax.plugin.rdbms.writer.Constant.OB10_SPLIT_STRING)) {
            ...
            logger.info("this is ob1_0 jdbc url. user=" + user + " :url=" + url);
        }
    ...
    public static List<String> getTableColumnsByConn(DataBaseType dataBaseType, Connection conn, String tableName, String basicMsg) {
        ...
        try {
            ...
            DBUtilNew.closeDBResources(rs, statement, conn);
        }
    ...
    public static Triple<List<String>, List<Integer>, List<String>> getColumnMetaData(
        ...
        try {
            ...
            DBUtilNew.closeDBResources(null, null, conn);
        }
    }
    ...
    public static Triple<List<String>, List<Integer>, List<String>> getColumnMetaData(
        ...
        try {
            ...
            DBUtilNew.closeDBResources(rs, statement, null);
        }
    }
    ...
    public static boolean testConnWithoutRetry(DataBaseType dataBaseType,
        ...
        try {
            ...
            logger.warn("test connection of [{}] failed, for {}.", url,
                    e.getMessage());
        } finally {
            DBUtilNew.closeDBResources(null, connection);
        }
        return false;
    }
            ...
            return RetryUtil.executeWithRetry(new Callable<Boolean>() {
                ...
                public Boolean call() throws Exception {
                    ...
                    try {
                        ...
                        if (DBUtilNew.asyncResultSetNext(rs, 5)) {
                        ...
                        DBUtilNew.closeDBResources(null, conn);
                    }
                }
            }, 3, 1000L, true);
        } catch (Exception e) {
    ...
    private static boolean doPreCheck(Connection conn, String pre) {
        ...
        try {
            ...
            logger.warn("pre check failed. pre:[{}], errorMessage:[{}].", pre,
                    e.getMessage());
        } finally {
            DBUtilNew.closeResultSet(rs);
        }
        return false;
    }
    ...
    public static void dealWithSessionConfig(Connection conn,
        ...
        switch (databaseType) {
            case Oracle:
                ...
                DBUtilNew.doDealWithSessionConfig(conn, sessionConfig, message);
                break;
            case DRDS:
                // 用于关闭 drds 的分布式事务开关
    ...
    private static void doDealWithSessionConfig(Connection conn,
        ...
        for (String sessionSql : sessions) {
            logger.info("execute sql:[{}]", sessionSql);
            try {
                DBUtilNew.executeSqlWithoutResultSet(stmt, sessionSql);
            } catch (SQLException e) {
                throw DataXException.asDataXException(
        ...
        DBUtilNew.closeDBResources(stmt, null);
    }
