...
public class HBase20xSQLWriterTask {
    private final static Logger logger = LoggerFactory.getLogger(HBase20xSQLWriterTask.class);
    ...
    private int[] getColumnSqlType() throws SQLException {
        ...
        try {
            ...
            for (int i = 0; i < columns.size(); i++) {
                ...
                logger.debug("Column name : " + name + ", sql type = " + types[i] + " " + meta.getColumnTypeName(i + 1));
            }
    ...
    private void doSingleUpsert(List<Record> records) throws SQLException {
        ...
        for (Record r : records) {
            try {
                ...
                //出错了，记录脏数据
                logger.error("Failed writing to phoenix, rowNumber: " + rowNumber);
