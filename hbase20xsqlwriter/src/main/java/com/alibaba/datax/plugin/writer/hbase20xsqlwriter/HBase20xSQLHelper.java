...
public class HBase20xSQLHelper {
    private static final Logger logger = LoggerFactory.getLogger(HBase20xSQLHelper.class);
    ...
    public static Connection getThinClientConnection(String connStr) {
        logger.debug("Connecting to QueryServer [" + connStr + "] ...");
        Connection conn;
        try {
    ...
    public static void checkTable(Connection conn, String schema, String tableName, List<String> columnNames) throws DataXException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                logger.error(tableName + "表不存在，请检查表名是否正确或是否已创建.", HBase20xSQLWriterErrorCode.GET_HBASE_TABLE_ERROR);
    ...
    public static void closeJdbc(Connection connection, Statement statement, ResultSet resultSet) {
        try {
            ...
            logger.warn("数据库连接关闭异常.", HBase20xSQLWriterErrorCode.CLOSE_HBASE_CONNECTION_ERROR);
        }
    }
}
