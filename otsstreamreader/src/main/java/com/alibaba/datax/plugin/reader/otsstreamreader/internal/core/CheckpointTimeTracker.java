...
public class CheckpointTimeTracker {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CheckpointTimeTracker.class);
    ...
    public int getShardCountForCheck(long timestamp) {
        ...
        logger.info("GetShardCount: timestamp: {}, shardCount: {}.", timestamp, shardCount);
        return shardCount;
    }
    ...
    public void setShardTimeCheckpoint(String shardId, long timestamp, String checkpointValue) {
        ...
        logger.info("SetShardTimeCheckpoint: timestamp: {}, shardId: {}, checkpointValue: {}.", timestamp, shardId, checkpointValue);
    }
    ...
    public String getShardLargestCheckpointInTimeRange(String shardId, long startTimestamp, long endTimestamp) {
        ...
        if (result.getRows().isEmpty()) {
            ...
            try {
                ...
                logger.info("find checkpoint for shard {} in time {}.", shardId, time);
                return checkpoint;
            } catch (Exception ex) {
                logger.error("Error when get shard time checkpoint.", ex);
                return null;
            }
        }
    }
    ...
    public void clearAllCheckpoints(long timestamp) {
        ...
        logger.info("ClearAllCheckpoints: timestamp: {}.", timestamp);
    }
    ...
    public void writeCheckpoint(long timestamp, ShardCheckpoint checkpoint, long sendRecordCount) {
        logger.info("Write checkpoint of time '{}' of shard '{}'.", timestamp, checkpoint.getShardId());
    ...
    public boolean getAndCheckAllCheckpoints(long timestamp, String streamId, Map<String, ShardCheckpoint> allCheckpoints) {
        ...
        if (streamJob == null) {
            logger.info("Stream job is not exist, timestamp: {}.", timestamp);

            // 如果streamJob不存在，则有可能是老版本的Job，尝试读取shardCount
            shardCount = getShardCountForCheck(timestamp);
            if (shardCount == -1) {
                logger.info("Shard count not found, timestamp: {}.", timestamp);
                return false;
            }
        ...
        if (checkShardCountOnly) {
            if (shardCount != allCheckpointsInTable.size()) {
                logger.info("Shard count not equal, shardCount: {}, checkpointCount: {}.", shardCount, allCheckpoints.size());
                return false;
            }
        } else {
            // 检查streamJob内的信息是否与checkpoint一致
            if (!streamJob.getStreamId().equals(streamId)) {
                logger.info("Stream id of the checkpoint is not equal with current job. StreamIdInCheckpoint: {}, StreamId: {}.",
            ...
            for (String shardId : streamJob.getShardIds()) {
                ...
                if (checkpoint == null) {
                    logger.info("Checkpoint of shard in job is not found. ShardId: {}.", shardId);
                    return false;
                }

                if (!checkpoint.getVersion().equals(streamJob.getVersion())) {
                    logger.info("Version is different. Checkpoint: {}, StreamJob: {}.", checkpoint, streamJob);
                    return false;
                }
            }
        }
