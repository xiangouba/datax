...
public class OTSStreamReaderSlaveProxy {
    private static final Logger logger = LoggerFactory.getLogger(OTSStreamReaderSlaveProxy.class);
    ...
    public void init(final OTSStreamReaderConfig otsStreamReaderConfig, StreamJob streamJob, List<StreamShard> allShards, Set<String> ownedShardIds) {
        ...
        logger.info("SlaveId: {}, ShardIds: {}, OwnedShards: {}.", slaveId, allShards, ownedShardIds);
        ...
        if (!findCheckpoints) {
            logger.info("Checkpoint for stream '{}' in timestamp '{}' is not found.", streamJob.getStreamId(), streamJob.getStartTimeInMillis());
            setWithNearestCheckpoint();
        }

        logger.info("Find checkpoints: {}.", findCheckpoints);
        for (Map.Entry<String, StreamShard> shard : ownedShards.entrySet()) {
            logger.info("Shard to process, ShardInfo: [{}], StartCheckpoint: [{}].", shard.getValue(), shardToCheckpointMap.get(shard.getKey()));
        }
        logger.info("Count of owned shards: {}. ShardIds: {}.", ownedShardIds.size(), ownedShardIds);
    }
    ...
    private void setWithNearestCheckpoint() {
        ...
        if (timeRangeBegin < timeRangeEnd) {
            for (String shardId : ownedShards.keySet()) {
                logger.info("Try find nearest checkpoint for shard {}, startTime: {}.", shardId, config.getStartTimestampMillis());
                String checkpoint = this.checkpointInfoTracker.getShardLargestCheckpointInTimeRange(shardId, timeRangeBegin, timeRangeEnd);
                if (checkpoint != null) {
                    logger.info("Found checkpoint for shard {}, checkpoint: {}.", shardId, checkpoint);
    ...
    private Map<String, StreamShard> filterShardsReachEnd(Map<String, StreamShard> ownedShards, Map<String, ShardCheckpoint> allCheckpoints) {
        ...
        for (Map.Entry<String, StreamShard> shard : ownedShards.entrySet()) {
            ...
            if (allCheckpoints.get(shardId).getCheckpoint().equals(CheckpointPosition.SHARD_END)) {
                logger.info("Shard has reach end, no need to process. ShardId: {}.", shardId);
    ...
    public void startRead(RecordSender recordSender) {
        ...
        logger.info("Start thread pool with size: {}, ShardsCount: {}, SlaveCount: {}.", threadPoolSize, ownedShards.size(), slaveNumber.get());
        try {
            ...
            while (!allShardToProcess.isEmpty()) {
                ...
                for (StreamShard shard : shardNoNeedProcess) {
                    logger.info("Skip shard: {}.", shard.getShardId());
                ...
                if (now - lastLogTime > config.getSlaveLoggingStatusInterval()) {
                    ...
                    logger.info("AllCheckpoints: {}", checkpointMap);
                    lastLogTime = now;
                }
            }

            logger.info("All shard is processing.");
            logShardProcessingState(shardProcessingState);
            ...
            while (true) {
                ...
                for (RecordProcessor processor : processors) {
                    ...
                    if (state != RecordProcessor.State.SUCCEED) {
                        logger.info("Shard is processing, shardId: {}, status: {}.", processor.getShard().getShardId(), state);
                        finished = false;
                    }
                }

                if (finished) {
                    logger.info("All record processor finished.");
                    break;
                }
            ...
            logger.error("SlaveId: {}, OwnedShards: {}.", slaveId, ownedShards, ex);
            ...
            try {
                ...
                logger.error("Shutdown encounter exception.", e);
            }
        }
    }
    ...
    private void logShardProcessingState(Map<String, ShardStatusChecker.ProcessState> shardProcessingState) {
        ...
        logger.info("Version: {}, Reader status: {}", streamJob.getVersion(), sb.toString());
    }
    ...
    void checkCheckpoint(Map<String, ShardCheckpoint> checkpointMap, StreamJob streamJob) {
        for (Map.Entry<String, ShardCheckpoint> entry : checkpointMap.entrySet()) {
            ...
            if (!streamJob.getShardIds().contains(shardId)) {
                logger.info("Shard '{}' is not found in job. Job: {}.", entry.getKey(), streamJob.getShardIds());
            ...
            if (!version.equals(streamJob.getVersion())) {
                logger.info("Version of shard '{}' in checkpoint is not equal with version of this job. " +
