...
public class RecordProcessor implements Runnable {
    ...
    public void initialize() {
        ...
        logger.info("Initialize record processor. Mode: {}, StartCheckpoint: [{}], ShardId: {}, ShardIterator: {}, SkipCount: {}.",
    ...
    @Override
    public void run() {
        logger.info("Start process records with startTime: {}, endTime: {}, nextShardIterator: {}, skipCount: {}.",
        ...
        try {
            ...
            logger.error("Some fatal error has happened, shardId: {}, LastShardIterator: {}, NextShartIterator: {}.",
                    shard.getShardId(), lastShardIterator, nextShardIterator, e);
            state = State.FAILED;
        }
        logger.info("Finished process records. ShardId: {}, RecordSent: {}.", shard.getShardId(), sendRecordCount.get());
    }
    ...
    boolean process(List<StreamRecord> records, String nextShardIterator) {
        if (records.isEmpty() && nextShardIterator != null) {
            logger.info("ProcessFinished: No more data in shard, shardId: {}.", shard.getShardId());
        ...
        for (int i = 0; i < size; i++) {
            ...
            logger.debug("Process record with timestamp: {}.", timestamp);
            if (timestamp < endTimestampMillis) {
                if (shouldSkip && (timestamp < startTimestampMillis)) {
                    logger.debug("Skip record out of start time: {}, startTime: {}.", timestamp, startTimestampMillis);
                    continue;
                }
                shouldSkip = false;
                if (skipCount > 0) {
                    logger.debug("Skip record. Timestamp: {}, SkipCount: {}.", timestamp, skipCount);
                    skipCount -= 1;
                    continue;
                }

                logger.debug("Send record. Timestamp: {}.", timestamp);
                sendRecord(records.get(i));
            } else {
                logger.info("ProcessFinished: Record in shard reach boundary of endTime, shardId: {}. Timestamp: {}, EndTime: {}", shard.getShardId(), timestamp, endTimestampMillis);
        ...
        if (nextShardIterator == null) {
            logger.info("ProcessFinished: Shard has reach to end, shardId: {}.", shard.getShardId());
    ...
    private boolean readAndProcessRecords() {
        logger.debug("Read and process records. ShardId: {}, ShardIterator: {}.", shard.getShardId(), nextShardIterator);
    ...
    public boolean processRecords(List<StreamRecord> records, String nextShardIterator) {
        ...
        if (records.isEmpty()) {
            logger.info("StartProcessRecords: size: {}.", records.size());
        } else {
            logger.debug("StartProcessRecords: size: {}, recordTime: {}.", records.size(), getTimestamp(records.get(0)));
        }
        ...
        logger.debug("ProcessRecords, ProcessShard:{}, ProcessTime: {}, Size:{}, NextShardIterator:{}",
