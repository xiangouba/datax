...
public class StandardFtpHelperImpl implements IFtpHelper {
    ...
    public Set<String> getAllFilesInDir(String dir, String prefixFileName) {
        ...
        try {
            ...
            // logger.debug(JSON.toJSONString(this.ftpClient.listNames(dir)));
            logger.debug(String.format("ls: %s",
    ...
    private void printWorkingDirectory() {
        try {
            logger.info(String.format("current working directory:%s",
                    this.ftpClient.printWorkingDirectory()));
        } catch (Exception e) {
            logger.warn(String.format("printWorkingDirectory error:%s",
                    e.getMessage()));
        }
    }
