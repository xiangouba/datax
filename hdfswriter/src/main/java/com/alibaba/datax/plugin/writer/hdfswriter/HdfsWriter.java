...
public class HdfsWriter extends Writer {
    public static class Job extends Writer.Job {
        ...
        public void prepare() {
            ...
            if(hdfsHelper.isPathexists(path)){
                ...
                /**
                 ...
                 logger.info(String.format("由于您配置了writeMode truncate, 开始清理 [%s] 下面以 [%s] 开头的内容",
                 path, fileName));
                 hdfsHelper.deleteFiles(existFilePaths);
                 } else
                 */
                if ("append".equalsIgnoreCase(writeMode)) {
                    logger.info(String.format("由于您配置了writeMode append, 写入前不做清理工作, [%s] 目录下写入相应文件名前缀  [%s] 的文件",
                            path, fileName));
                    ...
                    logger.info(String.format("由于您配置了writeMode truncate,  [%s] 下面的内容将被覆盖重写", path));
                    hdfsHelper.deleteFiles(existFilePaths);
                }
            }else{
                throw DataXException.asDataXException(HdfsWriterErrorCode.ILLEGAL_VALUE,
        ...
        public List<Configuration> split(int mandatoryNumber) {
            ...
            for (int i = 0; i < mandatoryNumber; i++) {
                ...
                logger.info(String.format("splited write file name:[%s]",
                        fullFileName));
        ...
        public void unitizeParquetConfig(Configuration writerSliceConfig) {
            ...
            if (StringUtils.isNotBlank(parquetSchema)) {
                logger.info("parquetSchema has config. use parquetSchema:\n{}", parquetSchema);
                return;
            }
            ...
            try {
                ...
                logger.warn("The generated parquetSchema {} is illegal, try to generate parquetSchema in another way", parquetSchema);
                parquetSchema = HdfsHelper.generateParquetSchemaFromColumnAndType(columns);
                logger.info("The last generated parquet schema is {}", parquetSchema);
            }
            writerSliceConfig.set(Key.PARQUET_SCHEMA, parquetSchema);
            logger.info("dataxParquetMode use default fields.");
        ...
        private String generateParquetSchemaFromColumn(List<Configuration> columns) {
            ...
            logger.info("generate parquetSchema:\n{}", parquetSchema);
    ...
    public static class Task extends Writer.Task {
        ...
        public void startWrite(RecordReceiver lineReceiver) {
            ...
            logger.info(String.format("write to file : [%s]", this.fileName));
            if(fileType.equalsIgnoreCase("TEXT")){
                //写TEXT FILE
