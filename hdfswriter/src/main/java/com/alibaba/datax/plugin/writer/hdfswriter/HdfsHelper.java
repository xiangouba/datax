...
public  class HdfsHelper {
    public static final Logger logger = LoggerFactory.getLogger(HdfsWriter.Job.class);
    ...
    public void deleteFiles(Path[] paths){
        for(int i=0;i<paths.length;i++){
            logger.info(String.format("delete file [%s].", paths[i].toString()));
            try {
    ...
    public void deleteDir(Path path){
        logger.info(String.format("start delete tmp dir [%s] .",path.toString()));
        try {
        ...
        logger.info(String.format("finish delete tmp dir [%s] .",path.toString()));
    }
        ...
        if(tmpFiles.size() != endFiles.size()){
            ...
            try{
                for (Iterator it1=tmpFiles.iterator(),it2=endFiles.iterator();it1.hasNext()&&it2.hasNext();){
                    ...
                    logger.info(String.format("start rename file [%s] to file [%s].", srcFile,dstFile));
                    ...
                    if(fileLen>0){
                        ...
                        logger.info(String.format("finish rename file [%s] to file [%s].", srcFile,dstFile));
                    }else{
                        logger.info(String.format("文件［%s］内容为空,请检查写入是否正常！", srcFile));
                    }
                }
            }catch (Exception e) {
