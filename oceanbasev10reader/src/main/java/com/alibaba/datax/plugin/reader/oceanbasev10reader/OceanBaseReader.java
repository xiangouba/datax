    ...
    public static class Job extends Reader.Job {
        ...
        public void init() {
            ...
            if (userConfigedFetchSize != null) {
                logger.warn("The [fetchSize] is not recognized, please use readBatchSize instead.");
            }
            ...
            if (splitPk != null && splitPk.length() > 0 && quotedColumns != null) {
                ...
                for (String column : quotedColumns) {
                    if (column.equals(splitPk)) {
                        logger.info("splitPk is an ob reserved keyword, set to {}", splitPk);
        ...
        private void setDatabaseType(Configuration config) {
            ...
            if (jdbcUrl.startsWith(com.alibaba.datax.plugin.rdbms.writer.Constant.OB10_SPLIT_STRING)) {
                ...
                if (ss.length != 3) {
                    logger.warn("unrecognized jdbc url: " + jdbcUrl);
                    return;
                }
            ...
            try {
                ...
                logger.warn("error in get compatible mode, using mysql as default: " + e.getMessage());
            }
        }
    }
