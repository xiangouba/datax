...
public class ObReaderUtils {
    private static final Logger logger = LoggerFactory.getLogger(ObReaderUtils.class);
    ...
    public static void initConn4Reader(Connection conn, long queryTimeoutSeconds) {
        ...
        try {
            ...
            logger.warn("setAutoCommit=true;" + setQueryTimeout + ";" + setTrxTimeout + ";");
        } catch (Throwable e) {
            logger.warn("initConn4Reader fail", e);
        } finally {
    ...
    public static void matchPkIndexs(Connection conn, TaskContext context) {
        ...
        if (ArrayUtils.isEmpty(pkColumns)) {
            logger.warn("table=" + context.getTable() + " has no primary key");
            return;
        }
    ...
    private static String[] getPkColumns(Connection conn, TaskContext context) {
        ...
        logger.info("get primary key by sql: " + sql);
        Statement ps = null;
        ResultSet rs = null;
        ...
        try {
            ...
            logger.error("show index from table fail :" + sql, e);
        } finally {
            close(rs, ps, null);
        }
    ...
    public static boolean isUserSavePointValid(TaskContext context) {
        String userSavePoint = context.getUserSavePoint();
        if (userSavePoint == null || userSavePoint.length() == 0) {
            logger.info("user save point is empty!");
            return false;
        }

        logger.info("validating user save point: " + userSavePoint);

        final String patternString = "(.+)=(.+)";
        Pattern parttern = Pattern.compile(patternString);
        Matcher matcher = parttern.matcher(userSavePoint);
        if (!matcher.find()) {
            logger.error("user save point format is not correct: " + userSavePoint);
            return false;
        }
        ...
        if (columnsInUserSavePoint.size() == 0 || valuesInUserSavePoint.size() == 0 ||
            ...
            logger.error("number of columns and values in user save point are different:" + userSavePoint);
            return false;
        }
        ...
        if (StringUtils.isNotEmpty(where)) {
            for (String column : columnsInUserSavePoint) {
                if (where.contains(column)) {
                    logger.error("column " + column + " is conflict with where: " + where);
                    return false;
                }
            }
        }
        ...
        if (pkColumns.length != columnsInUserSavePoint.size()) {
            logger.error("user save point is not on the selected index.");
            return false;
        }
        ...
        for (String column : columnsInUserSavePoint) {
            ...
            if (!found) {
                logger.error("column " + column + " is not on the selected index.");
                return false;
            }
        }
    ...
    private static List<String> getAllColumnFromTab(Connection conn, String tableName) {
        ...
        try {
            ...
            logger.warn("fail to get all columns from table " + tableName, e);
        } finally {
            close(rs, stmt, null);
        }

        logger.info("all columns in tab: " + String.join(",", allColumns));
        return allColumns;
    }
    ...
    private static Map<String, List<String>> getAllIndex(Connection conn, String tableName, String compatibleMode) {
        ...
        try {
            logger.info("running sql to get index: " + sql);
            stmt = conn.createStatement();
            rs = stmt.executeQuery(sql);
            while (rs.next()) {
            ...
            logger.error("fail to get all keys from table" + sql, e);
        } finally {
            close(rs, stmt, null);
        }

        logger.info("all index: " + allIndex.toString());
        return allIndex;
    }
    ...
    private static List<String> getIndexName(Connection conn, String table,
        ...
        if (colNamesInCondition == null || colNamesInCondition.size() == 0) {
            logger.info("there is no qulified conditions in the where clause, skip index selection.");
            return indexNames;
        }

        logger.info("columnNamesInConditions: " + String.join(",", colNamesInCondition));
    ...
    private static void findBestIndex(Connection conn, List<String> indexNames, String table, TaskContext context) {
        if (indexNames.size() == 0) {
            logger.warn("table has no index.");
            return;
        }
        ...
        logger.info("possible index:" + allIndexs + ",where:" + context.getWhere());
        ...
        if (chooseIndex != null) {
            logger.info("choose index name:" + chooseIndex.getKey() + ",columns:" + chooseIndex.getValue());
    ...
    public static String getCompatibleMode(Connection conn) {
        ...
        try {
            ...
            logger.error("fail to get ob compatible mode, using mysql as default: " + e.getMessage());
        } finally {
            DBUtil.closeDBResources(rs, stmt, conn);
        }

        logger.info("ob compatible mode is " + compatibleMode);
    ...
    public static String getDbNameFromJdbcUrl(String jdbcUrl) {
        ...
        if (matcher.find()) {
            ...
            logger.error("jdbc url {} is not valid.", jdbcUrl);
        }
