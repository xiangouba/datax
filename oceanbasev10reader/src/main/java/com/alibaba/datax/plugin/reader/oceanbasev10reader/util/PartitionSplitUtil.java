...
public class PartitionSplitUtil {
    private static final Logger logger = LoggerFactory.getLogger(PartitionSplitUtil.class);
    ...
    private static List<Configuration> splitSinglePartitionTable(Configuration configuration) {
        ...
        if (partInfo != null && partInfo.isPartitionTable()) {
            ...
            for (String partitionName : partInfo.getPartList()) {
                logger.info(String.format("add %s %s for table %s", partitionType, partitionName, table));
            ...
            logger.info("fail to get table part info or table is not partitioned, proceed as non-partitioned table.");
    ...
    private static PartInfo getObPartInfoBySQL(Configuration config, String table) {
        ...
        try {
            ...
            logger.info("obVersion: " + obVersion);
            ...
            logger.error("error when get partition list: " + ex.getMessage());
        } finally {
    ...
    private static List<String> getResultsFromSql(Connection conn, String sql) {
        ...
        try {
            ...
            logger.error("error when executing sql: " + e.getMessage());
        } finally {
