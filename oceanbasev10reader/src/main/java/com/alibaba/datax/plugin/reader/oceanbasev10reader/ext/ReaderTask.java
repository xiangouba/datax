...
public class ReaderTask extends CommonRdbmsReader.Task {
    ...
    public void init(Configuration readerSliceConfig) {
        ...
        logger.info("this is ob1_0 jdbc url. user=" + username + " :url=" + jdbcUrl);
    ...
    private void buildSavePoint(TaskContext context) {
        if (!ObReaderUtils.isUserSavePointValid(context)) {
            logger.info("user save point is not valid, set to null.");
    ...
    private void startRead0(boolean isTableMode, TaskContext context, RecordSender recordSender,
        ...
        try {
            ...
            logger.warn("fetch PkIndexs fail,table=" + context.getTable(), e);
        }
        ...
        logger.warn("start table scan key : {}", context.getIndexName() == null ? "primary" : context.getIndexName());
        ...
        while (true) {
            try {
                ...
                logger.error("read fail, retry count " + retryCount + ", sleep 60 second, save point:" +
    ...
    private boolean isConnectionAlive(Connection conn) {
        ...
        try {
            ...
            logger.info("connection is not alive: " + ex.getMessage());
            return false;
        } finally {
    ...
    private boolean doRead(RecordSender recordSender, TaskPluginCollector taskPluginCollector, TaskContext context) {
        logger.info("exe sql: {}", context.getQuerySql());
        Connection conn = context.getConn();
        if (reuseConn && isConnectionAlive(conn)) {
            logger.info("connection is alive, will reuse this connection.");
        } else {
            logger.info("Create new connection for reader.");
        ...
        try {
            ...
            if (context.getPkIndexs() != null && context.getSavePoint() != null) {
                ...
                if (logger.isWarnEnabled()) {
                    ...
                    logger.warn("{} save point : {}", context.getTable(), StringUtils.join(pointForLog, ','));
                }
            }
            // 打开流式接口
            ...
            logger.info("end of sql: {}, " + count + "rows are read.", context.getQuerySql());
            return context.getReadBatchSize() <= 0 || count < readBatchSize;
        } catch (Exception e) {
            ObReaderUtils.close(null, null, context.getConn());
            context.setConn(null);
            logger.error("reader data fail", e);
