    ...
    public static class Job extends Writer.Job {
        ...
        public void init() {
            ...
            if (StringUtils.isBlank(sourceDbType)) {
                ...
                logger.info("The parameter [" + Key.SOURCE_DB_TYPE + "] will be default value: " + SourceDBType.TSDB);
            }
            try {
            ...
            if (DB_TYPE == SourceDBType.TSDB) {
                ...
                if (retrySize == null || retrySize < 0) {
                    ...
                    logger.info("The parameter [" + Key.MAX_RETRY_TIME +
                ...
                if (ignoreWriteError == null) {
                    ...
                    logger.info("The parameter [" + Key.IGNORE_WRITE_ERROR +
                ...
                if (StringUtils.isBlank(database)) {
                    logger.info("The parameter [" + Key.DATABASE + "] is blank.");
                } else {
                	logger.warn("The parameter [" + Key.DATABASE + "] : {} is ignored.");
                    // tsdb.useDatabase(database);
                }

                logger.info("Tsdb config: {}", ConfigurationUtil.filterSensitive(originalConfig).toJSON());
    ...
    public static class Task extends Writer.Task {
        ...
        public void startWrite(RecordReceiver recordReceiver) {
            ...
            if (DB_TYPE == SourceDBType.TSDB) {
                ...
                logger.info("Task finished, write size: {}", total);
        ...
        private void batchPut(final Record record, final String dps) {
            try {
                ...
                if (ignoreWriteError) {
                    logger.warn("Ignore write exceptions and continue writing.");
                } else {
            ...
            if (ignoreWriteError) {
                if (multiField) {
                    ...
                    if (ignoreErrorsResult == null) {
                        logger.error("Unexpected inner error for insert");
                    } else if (ignoreErrorsResult.getFailed() > 0) {
                        logger.error("write TSDB failed num:" + ignoreErrorsResult.getFailed());
                    }
                } else {
                ...
                if (summaryResult.getFailed() > 0) {
                    logger.error("write TSDB failed num:" + summaryResult.getFailed());
