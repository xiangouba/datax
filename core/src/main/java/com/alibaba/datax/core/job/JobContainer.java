...
public class JobContainer extends AbstractContainer {
    ...
    @Override
    public void start() {
        logger.info("DataX jobContainer starts job.");
        ...
        try {
            ...
            if(isDryRun) {
                logger.info("jobContainer starts to do preCheck ...");
                this.preCheck();
            } else {
                userConf = configuration.clone();
                logger.debug("jobContainer starts to do preHandle ...");
                this.preHandle();

                logger.debug("jobContainer starts to do init ...");
                this.init();
                logger.info("jobContainer starts to do prepare ...");
                this.prepare();
                logger.info("jobContainer starts to do split ...");
                this.totalStage = this.split();
                logger.info("jobContainer starts to do schedule ...");
                this.schedule();
                logger.debug("jobContainer starts to do post ...");
                this.post();

                logger.debug("jobContainer starts to do postHandle ...");
                this.postHandle();
                logger.info("DataX jobId [{}] completed successfully.", this.jobId);

                this.invokeHooks();
            }
        } catch (Throwable e) {
            logger.error("Exception when job run", e);
            ...
            if(!isDryRun) {
                ...
                if (!hasException) {
                    ...
                    if (vmInfo != null) {
                        ...
                        logger.info(vmInfo.totalString());
                    }

                    logger.info(PerfTrace.getInstance().summarizeNoException());
                    this.logStatistics();
                }
            }
        }
    ...
    private void preCheckReader() {
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.READER, this.readerPluginName));
        logger.info(String.format("DataX Reader.Job [%s] do preCheck work .",
    ...
    private void preCheckWriter() {
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.WRITER, this.writerPluginName));
        logger.info(String.format("DataX Writer.Job [%s] do preCheck work .",
    ...
    private int split() {
        ...
        logger.debug("transformer configuration: "+ JSON.toJSONString(transformerList));
        /**
    ...
    private void adjustChannelNumber() {
        ...
        if (isByteLimit) {
            ...
            logger.info("Job set Max-Byte-Speed to " + globalLimitedByteSpeed + " bytes.");
        }
        ...
        if (isRecordLimit) {
            ...
            logger.info("Job set Max-Record-Speed to " + globalLimitedRecordSpeed + " records.");
        }
        ...
        if (isChannelLimit) {
            ...
            logger.info("Job set Channel-Number to " + this.needChannelNumber
                    + " channels.");
    ...
    private void schedule() {
        ...
        logger.info("Scheduler starts [{}] taskGroups.", taskGroupConfigs.size());
        ...
        try {
            ...
            logger.info("Running by {} Mode.", executeMode);
            ...
            logger.error("运行scheduler 模式[{}]出错.", executeMode);
            this.endTransferTimeStamp = System.currentTimeMillis();
            throw DataXException.asDataXException(
                    FrameworkErrorCode.RUNTIME_ERROR, e);
        }
    ...
    private void prepareJobReader() {
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.READER, this.readerPluginName));
        logger.info(String.format("DataX Reader.Job [%s] do prepare work .",
                this.readerPluginName));
    ...
    private void prepareJobWriter() {
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.WRITER, this.writerPluginName));
        logger.info(String.format("DataX Writer.Job [%s] do prepare work .",
                this.writerPluginName));
    ...
    private List<Configuration> doReaderSplit(int adviceNumber) {
        ...
        logger.info("DataX Reader.Job [{}] splits to [{}] tasks.",
    ...
    private void postJobReader() {
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.READER, this.readerPluginName));
        logger.info("DataX Reader.Job [{}] do post work.",
                this.readerPluginName);
        this.jobReader.post();
        classLoaderSwapper.restoreCurrentThreadClassLoader();
    }
    ...
    private void postJobWriter() {
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.WRITER, this.writerPluginName));
        logger.info("DataX Writer.Job [{}] do post work.",
                this.writerPluginName);
        this.jobWriter.post();
        classLoaderSwapper.restoreCurrentThreadClassLoader();
    }
