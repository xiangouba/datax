...
public class TransformerRegistry {
    ...
    private static final Logger logger = LoggerFactory.getLogger(TransformerRegistry.class);
    ...
    static {
        ...
        //加密
        registTransformer(new EncryptionTransformer());
        //json解析
        registTransformer(new JsonTransformer());
    }
    ...
    public static void loadTransformerFromLocalStorage(List<String> transformers) {
        ...
        for (final String each : paths) {
            try {
                ...
                logger.error(String.format("skip transformer(%s) loadTransformer has Exception(%s)", each, e.getMessage()), e);
            }
    ...
    public static void loadTransformer(String each) {
        ...
        try {
            ...
            logger.error(String.format("skip transformer(%s),load transformer.json error, path = %s, ", each, transformerPath), e);
            return;
        }

        String className = transformerConfiguration.getString("class");
        if (StringUtils.isEmpty(className)) {
            logger.error(String.format("skip transformer(%s),class not config, path = %s, config = %s", each, transformerPath, transformerConfiguration.beautify()));
            return;
        }

        String funName = transformerConfiguration.getString("name");
        if (!each.equals(funName)) {
            logger.warn(String.format("transformer(%s) name not match transformer.json config name[%s], will ignore json's name, path = %s, config = %s", each, funName, transformerPath, transformerConfiguration.beautify()));
        }
        ...
        try {
            ...
            if (ComplexTransformer.class.isAssignableFrom(transformer.getClass())) {
                ...
                logger.error(String.format("load Transformer class(%s) error, path = %s", className, transformerPath));
            }
        } catch (Exception e) {
            //错误funciton跳过
            logger.error(String.format("skip transformer(%s),load Transformer class error, path = %s ", each, transformerPath), e);
        }
    }
