...
public class AdsClientProxy implements AdsProxy {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AdsClientProxy.class);
    ...
    public AdsClientProxy(String table, List<String> columns, Configuration configuration,
        ...
        databaseConfig.setLogger(AdsClientProxy.logger);
        ...
        // 避免敏感信息直接打印
        logger.info("Adb database config is : {}", JSON.toJSONString(databaseConfig));
    ...
    public void closeResource() {
        try {
            logger.info("stop the adbClient");
            this.adbClient.stop();
        } catch (Exception e) {
            logger.warn("stop adbClient meet a exception, ignore it: {}", e.getMessage(), e);
        }
    }
}
