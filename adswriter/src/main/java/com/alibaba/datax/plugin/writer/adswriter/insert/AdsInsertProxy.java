...
public class AdsInsertProxy implements AdsProxy {
    ...
    private void doBatchRecord(final List<Record> buffer, final String mode) throws SQLException {
        ...
        try {
            ...
            logger.warn(String.format("after retry %s times, doBatchRecord meet a exception: ", this.retryTimeUpperLimit), e);
            logger.info("try to re execute for each record...");
    ...
    private void doBatchRecordDml(List<Record> buffer, String mode) throws Exception {
        ...
        try {
            ...
            logger.warn("doBatchRecordDml meet a exception: " + sql, e);
            ...
            while (null != eachException && maxIter < AdsInsertProxy.MAX_EXCEPTION_CAUSE_ITER) {
                if (this.isRetryable(eachException)) {
                    logger.warn("doBatchRecordDml meet a retry exception: " + e.getMessage());
                    ...
                    try {
                        ...
                        logger.warn("doBatchRecordDml meet a no! retry exception: " + e.getMessage());
                        throw e;
                    }
                }
                maxIter++;
    ...
    private void doOneRecordDml(Record record, String mode) throws Exception {
        ...
        try {
            ...
            logger.error("doOneDml meet a exception: " + sql, e);
            ...
            while (null != eachException && maxIter < AdsInsertProxy.MAX_EXCEPTION_CAUSE_ITER) {
                if (this.isRetryable(eachException)) {
                    logger.warn("doOneDml meet a retry exception: " + e.getMessage());
                    this.currentConnection = AdsUtil.getAdsConnect(this.configuration);
                    throw eachException;
                } else {
                    try {
                        ...
                        logger.warn("doOneDml meet a no! retry exception: " + e.getMessage());
                        throw e;
                    }
                }
                maxIter++;
