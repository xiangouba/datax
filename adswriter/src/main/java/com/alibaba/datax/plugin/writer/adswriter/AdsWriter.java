...
public class AdsWriter extends Writer {
    ...
    public static class Job extends Writer.Job {
        private static final Logger logger = LoggerFactory.getLogger(Writer.Job.class);
        ...
        public void init() {
            ...
            if(null == this.writeMode) {
                logger.warn("您未指定[writeMode]参数,  默认采用load模式, load模式只能用于离线表");
        ...
        private void loadAdsData(AdsHelper helper, String odpsTableName, String odpsPartition) {
            ...
            try {
                ...
                logger.info("ADS Load Data任务已经提交，job id: " + id);
                boolean terminated = false;
                int time = 0;
                while(!terminated) {
                    Thread.sleep(120000);
                    ...
                    time += 2;
                    logger.info("ADS 正在导数据中，整个过程需要20分钟以上，请耐心等待,目前已执行 "+ time+" 分钟");
                }
                logger.info("ADS 导数据已成功");
            } catch (AdsException e) {
                if (super.getPeerPluginName().equals(ODPS_READER)) {
                    // TODO 使用云账号
    ...
    public static class Task extends Writer.Task {
        private static final Logger logger = LoggerFactory.getLogger(Writer.Task.class);
