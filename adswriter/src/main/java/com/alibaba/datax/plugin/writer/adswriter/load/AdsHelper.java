...
public class AdsHelper {
    ...
    public TableInfo getTableInfo(String table) throws AdsException {
        ...
        try {
            ...
            logger.info(String.format("检查列信息sql语句:%s", columnMetaSql));
            ...
            logger.info(String.format("检查表信息sql语句:%s", tableMetaSql));
            statement = connection.createStatement();
    ...
    public String loadData(String table, String partition, String sourcePath, boolean overwrite) throws AdsException {
        ...
        try {
            ...
            connection = DriverManager.getConnection(url, connectionProps);
            statement = connection.createStatement();
            logger.info("正在从ODPS数据库导数据到ADS中: "+sb.toString());
            logger.info("由于ADS的限制，ADS导数据最少需要20分钟，请耐心等待");
