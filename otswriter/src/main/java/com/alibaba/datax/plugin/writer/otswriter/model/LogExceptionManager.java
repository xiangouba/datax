...
public class LogExceptionManager {
    ...
    private static final Logger logger = LoggerFactory.getLogger(LogExceptionManager.class);
    ...
    private synchronized void countAndReset() {
        ...
        if (interval >= 10000) {
            logger.warn("Call callable fail, OTSNotEnoughCapacityUnit, total times:"+ count +", time range:"+ (interval/1000) +"s, times per second:" + ((float)count / (interval/1000)));
            count = 0;
            updateTimestamp = cur;
        }
    }
    ...
    public synchronized void addException(Exception exception) {
        if (exception instanceof OTSException) {
            ...
            logger.warn("Call callable fail, {}", exception.getMessage());
        }
    }
