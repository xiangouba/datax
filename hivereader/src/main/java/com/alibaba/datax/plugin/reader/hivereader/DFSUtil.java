...
//    private static final Logger logger = LoggerFactory.getLogger(HiveReader.Job.class);
//
...
//        Configuration hadoopSiteParams = taskConfig.getConfiguration(KeyOld.HADOOP_CONFIG);
//        JSONObject hadoopSiteParamsAsJsonObject = JSON.parseObject(taskConfig.getString(KeyOld.HADOOP_CONFIG));
...
//            }
//        }
//        hadoopConf.set(HDFS_DEFAULTFS_KEY, taskConfig.getString(KeyOld.DEFAULT_FS));
//
//        //是否有Kerberos认证
//        this.haveKerberos = taskConfig.getBool(KeyOld.HAVE_KERBEROS, false);
//        if (haveKerberos) {
//            this.kerberosKeytabFilePath = taskConfig.getString(KeyOld.KERBEROS_KEYTAB_FILE_PATH);
//            this.kerberosPrincipal = taskConfig.getString(KeyOld.KERBEROS_PRINCIPAL);
//            this.hadoopConf.set(HADOOP_SECURITY_AUTHENTICATION_KEY, "kerberos");
//        }
//        this.kerberosAuthentication(this.kerberosPrincipal, this.kerberosKeytabFilePath);
//
//        logger.info(String.format("hadoopConfig details:%s", JSON.toJSONString(this.hadoopConf)));
//    }
//
...
//                logger.info(String.format("get HDFS all files in path = [%s]", eachPath));
...
//            // 判断是不是目录，如果是目录，递归调用
//            if (f.isDirectory()) {
//                logger.info(String.format("[%s] 是目录, 递归获取该目录下的文件", f.getPath().toString()));
//                getHDFSAllFilesNORegex(f.getPath().toString(), hdfs);
//            } else if (f.isFile()) {
//
//                addSourceFileByType(f.getPath().toString());
...
//            logger.info(String.format("[%s]是[%s]类型的文件, 将该文件加入source files列表", filePath, this.specifiedFileType));
...
//        logger.info(String.format("Start Read sequence file [%s].", sourceSequenceFilePath));
//
...
//            logger.info("Finally, Close stream SequenceFile.Reader.");
//        }
//
//    }
//
...
//                    recordReader.close();
//                    logger.info("Finally, Close RCFileRecordReader.");
//                }
//            } catch (IOException e) {
//                logger.warn(String.format("finally: 关闭RCFileRecordReader失败, %s", e.getMessage()));
//            }
//        }
//
//    }
...
//                                 RecordSender recordSender, TaskPluginCollector taskPluginCollector) {
//        logger.info(String.format("Start Read orcfile [%s].", sourceOrcFilePath));
//        List<ColumnEntry> column = UnstructuredStorageReaderUtil
//                .getListColumnEntry(readerSliceConfig, com.alibaba.datax.plugin.unstructuredstorage.reader.Key.COLUMN);
//        String nullFormat = readerSliceConfig.getString(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.NULL_FORMAT);
...
//            }
//        } catch (IOException e) {
//            logger.info(String.format("检查文件类型: [%s] 不是ORC File.", file.toString()));
//        }
//        return false;
//    }
//
...
//            return true;
//        } catch (IOException e) {
//            logger.info(String.format("检查文件类型: [%s] 不是RC File.", filepath));
//        }
//        return false;
//    }
//
...
//            }
//        } catch (IOException e) {
//            logger.info(String.format("检查文件类型: [%s] 不是Sequence File.", filepath));
//        }
//        return false;
//    }
//
...
//     * @param path
//     */
//    public void mkdirDirectory(String path) {
//        FileSystem fs = null;
//        try {
//            if (!exists(path)) {
//                fs = FileSystem.get(hadoopConf);
//                //创建文件夹
//                boolean m = fs.mkdirs(new Path(path));
//                logger.info(String.format("新建 hdfs 目录: %s ,%s"), path, m);
//            } else {
//                logger.info("hdfs 目录存在：" + path);
//            }
//        } catch (Exception e) {
