...
public class OdpsReader extends Reader {
    public static class Job extends Reader.Job {
        ...
        private void dealPartition(Table table) {
            ...
            if (isPartitionedTable) {
                logger.info(MESSAGE_SOURCE.message("odpsreader.7", table.getName(),
        ...
        private List<String> expandNoHintUserConfiguredPartition(List<String> allPartitions,
            ...
            logger.info("user configured partition: {}", JSON.toJSONString(userNormalConfiguredPartitions));
            logger.info("formated partition: {}", JSON.toJSONString(allStandardUserConfiguredPartitions));
        ...
        private void dealColumn(Table table) {
            ...
            logger.info(MESSAGE_SOURCE.message("odpsreader.10", table.getName(), columnMeta.toString()));

            if (1 == userConfiguredColumns.size()
                && "*".equals(userConfiguredColumns.get(0))) {
                logger.warn(MESSAGE_SOURCE.message("odpsreader.11"));
            ...
            sb.append(" ]");
            logger.info("parsed column details: {} .", sb.toString());
        }
        ...
        public void prepare() {
            ...
            if (preSqls != null && !preSqls.isEmpty()) {
                ...
                if (StringUtils.isNotBlank(preSqlBuffer.toString())) {
                    ...
                    logger.info("skip to execute the preSql: {}", JSON.toJSONString(preSqls));
                }
        ...
        public void post() {
            ...
            if (postSqls != null && !postSqls.isEmpty()) {
                ...
                if (StringUtils.isNotBlank(postSqlBuffer.toString())) {
                    ...
                    logger.info("skip to execute the postSql: {}", JSON.toJSONString(postSqls));
                }
    ...
    public static class Task extends Reader.Task {
        ...
        public void init() {
            ...
            if (this.isPartitionedTable && StringUtils.isBlank(partition) && this.successOnNoPartition) {
                logger.warn("Partition is blank, but you config successOnNoPartition[true] ,don't need to create session");
        ...
        public void startRead(RecordSender recordSender) {
            ...
            if (count > 0) {
                ...
                logger.warn(MESSAGE_SOURCE.message("odpsreader.12", this.tableName, partition));
                return;
            } else {
