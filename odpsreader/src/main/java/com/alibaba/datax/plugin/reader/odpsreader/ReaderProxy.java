...
public class ReaderProxy {
    ...
    private static boolean IS_DEBUG = logger.isDebugEnabled();
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    //odps read 异常后重试10次
                    logger.warn("warn : odps read exception: {}", e.getMessage());
                    if(retryTimes < 10) {
                        try {
                            Thread.sleep(2000);
                        ...
                        logger.warn(MESSAGE_SOURCE.message("readerproxy.1", retryTimes));
                        retryTimes++;
                        continue;
                    } else {
    ...
    private Map<String, String> parseCurrentPartitionValue() {
        ...
        if (IS_DEBUG) {
            logger.debug(String.format("partition value details: %s",
